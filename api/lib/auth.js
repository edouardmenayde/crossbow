...
export default function withAuth(scope, callback) {
  ...
  return async function (_, __, context) {
    ...
    if (
      (Array.isArray(requiredScope) && requiredScope.length && !context.auth.scope) ||
      (Array.isArray(requiredScope) && requiredScope.length && !validateScope(requiredScope, context.auth.scope))
    ) {
